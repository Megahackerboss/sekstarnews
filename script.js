...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showView(viewToShow) { /* ... bez zmian ... */ }
    function showMainView() { /* ... bez zmian ... */ }
    function renderComments(comments) { /* ... bez zmian z ostatniej działającej wersji ... */ }
    ...
    function loadInitialArticles() {
        ...
        query.once('value', (snapshot) => {
            ...
            if (newArticles.length < ARTICLES_PER_PAGE) { state.areAllArticlesLoaded = true; elements.loadMoreArticlesBtn.classList.add('hidden'); } else { elements.loadMoreArticlesBtn.classList.remove('hidden'); }
        });
    }
    ...
    function loadMoreArticles() {
        ...
        query.once('value', snapshot => {
            const data = snapshot.val();
            if (!data || Object.keys(data).length === 0) { state.areAllArticlesLoaded = true; elements.loadMoreArticlesBtn.classList.add('hidden'); return; }
            const newArticles = Object.values(data);
            ...
            if (newArticles.length < ARTICLES_PER_PAGE) { state.areAllArticlesLoaded = true; elements.loadMoreArticlesBtn.classList.add('hidden'); }
        });
    }

    function loadMoreComments() {
        if (state.areAllCommentsLoaded || !state.currentArticle) return;
        elements.loadMoreCommentsBtn.disabled = true;
        elements.loadMoreCommentsBtn.textContent = 'Ładowanie...';
        let query = database.ref(`comments/${state.currentArticle.id}`).orderByKey();
        if (state.lastLoadedCommentKey) { query = query.endBefore(state.lastLoadedCommentKey); }
        query = query.limitToLast(COMMENTS_PER_PAGE);
        query.once('value', (snapshot) => {
            const data = snapshot.val();
            if (!data) { state.areAllCommentsLoaded = true; elements.loadMoreCommentsBtn.classList.add('hidden'); if(state.displayedComments.length === 0) elements.commentSection.list.innerHTML = '<p>Brak komentarzy.</p>'; return; }
            ...
            if (newComments.length < COMMENTS_PER_PAGE) { state.areAllCommentsLoaded = true; elements.loadMoreCommentsBtn.classList.add('hidden'); } else { elements.loadMoreCommentsBtn.classList.remove('hidden'); elements.loadMoreCommentsBtn.disabled = false; elements.loadMoreCommentsBtn.textContent = 'Wczytaj więcej komentarzy'; }
        });
    }

    // Pozostałe funkcje Firebase (setupLikes, addComment, etc.) bez zmian...
