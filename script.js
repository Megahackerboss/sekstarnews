 ...
 * Wersja z zaawansowanym logowaniem diagnostycznym.
 */
document.addEventListener('DOMContentLoaded', () => {

    // =================================================================
    // === 0. SYSTEM DIAGNOSTYCZNY ======================================
    // =================================================================
    const Logger = {
        log: (message, ...args) => console.log(`[INFO] ${message}`, ...args),
        warn: (message, ...args) => console.warn(`[WARN] ${message}`, ...args),
        error: (message, ...args) => console.error(`[BŁĄD] ${message}`, ...args),
    };
    ...
    try {
        ...
        Logger.log("Firebase zainicjalizowany pomyślnie.");
        ...
        const elements = {
            ...
            views: { main: document.getElementById('main-view'), article: document.getElementById('article-view'), editor: document.getElementById('editor-view'), login: document.getElementById('login-view'), commentEditor: document.getElementById('comment-editor-view') },
            ...
            commentEditor: { textarea: document.getElementById('comment-editor-textarea'), saveButton: document.getElementById('comment-editor-save'), cancelButton: document.getElementById('comment-editor-cancel') },
        ...
        Logger.log("Wszystkie elementy DOM pobrane.");

        // =================================================================
        // === 3. STAN APLIKACJI ===========================================
        // =================================================================

        let state = { allArticlesMeta: [], lastLoadedArticleOrder: null, areAllArticlesLoaded: false, displayedComments: [], lastLoadedCommentKey: null, areAllCommentsLoaded: false, currentArticle: null, isUserAdmin: false, commentsListener: null, sliderInterval: null, currentSlideIndex: 0, localUserId: null };
        ...
        function showMainView() { Logger.log("Wyświetlanie widoku głównego."); showView(elements.views.main); elements.backButton.classList.add('hidden'); elements.navTitle.style.marginLeft = '0px'; startSlideInterval(); state.currentArticle = null; if (window.location.hash) window.location.hash = ''; }
        function displayNewsList(articles) { elements.newsList.innerHTML = ''; articles.forEach(article => { const card = document.createElement('div'); card.className = 'article-card'; card.dataset.id = article.id; card.innerHTML = `<img src="${article.thumbnail}" alt="${article.title}"><div class="article-card-content"><h4>${article.title}</h4></div>`; elements.newsList.appendChild(card); }); Logger.log(`Wyrenderowano ${articles.length} artykułów na liście.`); }
        async function displayArticle(articleId) { Logger.log(`Próba wyświetlenia artykułu o ID: ${articleId}`); let articleMeta = state.allArticlesMeta.find(a => a.id == articleId); if (!articleMeta) { try { const snapshot = await database.ref(`articles_meta/${articleId}`).once('value'); articleMeta = snapshot.val(); if (!articleMeta) { Logger.warn(`Nie znaleziono metadanych dla artykułu ID: ${articleId}. Powrót do strony głównej.`); showMainView(); return; } state.allArticlesMeta.push(articleMeta); } catch (error) { Logger.error(`Błąd pobierania metadanych artykułu ID: ${articleId}`, error); showMainView(); return; } } state.currentArticle = articleMeta; if (state.commentsListener) state.commentsListener.off(); elements.articleDetail.date.textContent = articleMeta.date; elements.articleDetail.author.textContent = `Autor: ${articleMeta.author}`; elements.articleDetail.content.innerHTML = '<p>Ładowanie treści...</p>'; showView(elements.views.article); elements.backButton.classList.remove('hidden'); elements.navTitle.style.marginLeft = '0px'; clearInterval(state.sliderInterval); const cachedArticle = JSON.parse(localStorage.getItem(`article_${articleId}`)); if (cachedArticle && cachedArticle.lastUpdated >= articleMeta.lastUpdated) { elements.articleDetail.content.innerHTML = cachedArticle.content; Logger.log(`Załadowano treść artykułu ID: ${articleId} z pamięci podręcznej.`); } else { database.ref(`articles_content/${articleId}`).once('value', (snapshot) => { const articleContent = snapshot.val(); if (articleContent) { elements.articleDetail.content.innerHTML = articleContent.content; localStorage.setItem(`article_${articleId}`, JSON.stringify({ content: articleContent.content, lastUpdated: articleMeta.lastUpdated })); Logger.log(`Załadowano treść artykułu ID: ${articleId} z Firebase i zapisano w cache.`); } else { Logger.warn(`Nie znaleziono treści dla artykułu ID: ${articleId}.`); } }); } state.displayedComments = []; state.lastLoadedCommentKey = null; state.areAllCommentsLoaded = false; elements.commentSection.list.innerHTML = ''; loadMoreComments(); setupLikes(articleId); setupShareButton(articleMeta); }
        function renderComments(comments) { const commentListContainer = elements.commentSection.list; const newList = document.createElement('div'); if (comments.length === 0) { newList.innerHTML = '<p>Brak komentarzy. Bądź pierwszy!</p>'; } else { comments.forEach(comment => { const commentEl = document.createElement('div'); commentEl.className = 'comment'; commentEl.dataset.commentId = comment.commentId; let controls = ''; if (state.isUserAdmin || comment.userId === state.localUserId) { controls = `<div class="comment-controls"><button class="edit-comment-btn">Edytuj</button><button class="delete-comment-btn">Usuń</button></div>`; } commentEl.innerHTML = `<div class="comment-header"><span class="comment-author">${comment.author || 'Anonim'}</span><span class="comment-date">${new Date(comment.timestamp).toLocaleString()}</span></div><p class="comment-message">${parseCommentFormatting(comment.message || '')}</p>${controls}`; newList.appendChild(commentEl); }); } commentListContainer.innerHTML = ''; commentListContainer.appendChild(newList); if (commentListContainer.eventListener) { commentListContainer.removeEventListener('click', commentListContainer.eventListener); } const eventHandler = (event) => { const target = event.target; const commentEl = target.closest('.comment'); if (!commentEl) return; const commentId = commentEl.dataset.commentId; if (target.classList.contains('delete-comment-btn')) { if (confirm("Czy na pewno chcesz usunąć ten komentarz?")) { database.ref(`comments/${state.currentArticle.id}/${commentId}`).remove(); } } if (target.classList.contains('edit-comment-btn')) { const commentData = comments.find(c => c.commentId === commentId); const messageP = commentEl.querySelector('.comment-message'); const controlsDiv = commentEl.querySelector('.comment-controls'); const editInput = document.createElement('textarea'); editInput.className = 'comment-edit-textarea'; editInput.value = commentData.message; const saveBtn = document.createElement('button'); saveBtn.textContent = 'Zapisz'; const cancelBtn = document.createElement('button'); cancelBtn.textContent = 'Anuluj'; messageP.style.display = 'none'; controlsDiv.style.display = 'none'; commentEl.appendChild(editInput); commentEl.appendChild(saveBtn); commentEl.appendChild(cancelBtn); editInput.focus(); saveBtn.onclick = () => { const newText = editInput.value.trim(); if (newText) { database.ref(`comments/${state.currentArticle.id}/${commentId}/message`).set(newText); } }; cancelBtn.onclick = () => { messageP.style.display = ''; controlsDiv.style.display = ''; editInput.remove(); saveBtn.remove(); cancelBtn.remove(); }; } }; commentListContainer.addEventListener('click', eventHandler); commentListContainer.eventListener = eventHandler; }
        ...
        function loadInitialArticles() { Logger.log("Ładowanie początkowych artykułów..."); let query = database.ref('articles_meta').orderByChild('order').limitToFirst(ARTICLES_PER_PAGE); query.once('value', (snapshot) => { const data = snapshot.val(); if (!data) { elements.loadMoreArticlesBtn.classList.add('hidden'); Logger.log("Brak artykułów w bazie danych."); return; } const newArticles = Object.values(data); state.allArticlesMeta = newArticles.sort((a, b) => (a.order || 999) - (b.order || 999)); state.lastLoadedArticleOrder = state.allArticlesMeta[state.allArticlesMeta.length - 1].order; displayNewsList(state.allArticlesMeta); const featured = state.allArticlesMeta.filter(a => a.featured); setupFeaturedSlider(featured); if (newArticles.length < ARTICLES_PER_PAGE) { state.areAllArticlesLoaded = true; elements.loadMoreArticlesBtn.classList.add('hidden'); } else { elements.loadMoreArticlesBtn.classList.remove('hidden'); } Logger.log(`Załadowano ${newArticles.length} artykułów.`); }).catch(err => Logger.error("Błąd podczas ładowania początkowych artykułów:", err)); }
        ...
        function initializeAuth() { auth.onAuthStateChanged(user => { state.isUserAdmin = user && !user.isAnonymous; elements.adminButton.textContent = state.isUserAdmin ? "≡" : "?"; if (!user) auth.signInAnonymously().catch(err => Logger.error("Błąd logowania anonimowego:", err)); }); }
        ...
        function handleDeepLink() { const hash = window.location.hash; if (hash && hash.startsWith('#article-')) { const articleId = hash.substring(9); if (state.allArticlesMeta.length > 0) { displayArticle(articleId); } else { Logger.log("Oczekiwanie na załadowanie artykułów przed otwarciem deep linka..."); setTimeout(() => handleDeepLink(), 200); } } }
        ...
        function bindEventListeners() {
            Logger.log("Wiązanie nasłuchiwaczy zdarzeń...");
            ...
            Logger.log("Nasłuchiwacze zdarzeń powiązane.");
        }
        ...
        function init() {
            Logger.log("Inicjalizacja aplikacji...");
            state.localUserId = getOrCreateLocalUserId();
            bindEventListeners();
            initializeAuth();
            loadInitialArticles();
            Logger.log("Aplikacja zainicjowana. Wygenerowane ID użytkownika:", state.localUserId);
        }
        
        init();

    } catch (error) {
        // Złapie każdy błąd, który wystąpi podczas inicjalizacji
        Logger.error("KRYTYCZNY BŁĄD PODCZAS INICJALIZACJI:", error);
        document.body.innerHTML = `<h1>Wystąpił krytyczny błąd. Sprawdź konsolę (F12), aby zobaczyć szczegóły.</h1>`;
    }
});
