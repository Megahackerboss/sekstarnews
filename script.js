window.addEventListener('load', () => {
    ...
    function updateContent() {
        ...
        if(elements.langSwitcher.flag) elements.langSwitcher.flag.src = lang === 'pl' ? "https://flagcdn.com/24x18/pl.png" : "https://flagcdn.com/24x18/gb.png";
        if(state.currentUser && elements.userPanel.addNewArticleBtn) elements.userPanel.addNewArticleBtn.textContent = i18next.t('user_panel.add_new_article');
    }
    ...
    function populateRoleSelect() {
        const sel = elements.userPanel.adminRoleSelect; if(!sel) return; sel.innerHTML = '';
        Object.keys(state.rolesConfig).forEach(r => sel.innerHTML += `<option value="${r}">${r}</option>`);
    }
    ...
    function setupFeaturedSlider(articles) {
        ...
        articles.forEach((article, index) => {
            const slide = document.createElement('div'); slide.className = 'slide'; slide.dataset.id = article.id;
            slide.innerHTML = `<img src="${article.thumbnail}"><div class="slide-title">${article.title}</div>`;
            content.appendChild(slide);
            const navDot = document.createElement('span'); navDot.className = 'nav-dot'; navDot.dataset.index = index;
            navDot.onclick = (e) => { e.stopPropagation(); showSlide(index); startSlideInterval(); };
            nav.appendChild(navDot);
        });
        showSlide(0); startSlideInterval();
    }
    function showSlide(index) {
        const slides = elements.slider.container.querySelectorAll('.slide'); const dots = elements.slider.container.querySelectorAll('.nav-dot');
    ...
    function displayNewsList(articles) {
        ...
        articles.forEach(article => {
            const card = document.createElement('div'); card.className = 'article-card'; card.dataset.id = article.id;
    ...
    function showMainView() {
        ...
        elements.views.main.classList.remove('hidden'); elements.backButton.classList.add('hidden');
        state.currentArticle = null; updateUIForPermissions();
    }
    async function displayArticle(id) {
        let meta = state.allArticlesMeta.find(a => a.id == id);
        if(!meta) try { const s = await database.ref(`articles_meta/${id}`).once('value'); meta = s.val(); } catch(e){}
        if(!meta) return showMainView();
        
        state.currentArticle = meta;
        Object.values(elements.views).forEach(v => v.classList.add('hidden'));
        elements.views.article.classList.remove('hidden'); elements.backButton.classList.remove('hidden');
        updateUIForPermissions();

        elements.articleDetail.date.textContent = meta.date; elements.articleDetail.author.textContent = meta.author;
        ...
        database.ref(`articles/${id}/likes`).on('value', s => {
            ...
            try { const liked = localStorage.getItem(`liked_${id}`) === 'true';
                elements.articleDetail.likeButton.querySelector('.heart-icon').textContent = liked ? '♥️' : '♡';
            } catch(e){}
        });
        database.ref(`articles_content/${id}`).once('value', s => elements.articleDetail.content.innerHTML = s.val() ? s.val().content : '');
        
        if(state.activeCommentsRef) state.activeCommentsRef.off();
        state.activeCommentsRef = database.ref(`comments/${id}`);
        state.activeCommentsRef.on('value', s => renderComments(s.val()));
    }
    ...
    function renderComments(data) {
        ...
        if(!data) return;
        const comments = Object.entries(data).map(([k,v]) => ({...v, id: k})).sort((a,b) => b.timestamp - a.timestamp);
        comments.forEach(comment => {
            const el = document.createElement('div'); el.className = 'comment'; el.dataset.commentId = comment.id;
            ...
            el.innerHTML = `
                <div class="comment-header"><span style="color:${comment.userColor || '#fff'}">${comment.author}</span><span class="comment-date">${new Date(comment.timestamp).toLocaleString()}</span></div>
                <p class="comment-message">${parseCommentFormatting(comment.message)}</p>${controls}
            `;
    ...
    function bindEvents() {
        ...
        const switchTab = (clicked, show, allBtns, allCont) => {
            Object.values(allBtns).forEach(b => b.classList.remove('active')); Object.values(allCont).forEach(c => c.classList.add('hidden'));
...
function saveArticle(e) {
        ...
        elements.editorForm.deleteButton.onclick = () => {
            ...
            if(confirm(i18next.t('editor.confirm_delete') || "Usunąć?")) {
                const updates = {}; updates[`/articles_meta/${id}`] = null; updates[`/articles_content/${id}`] = null;
                database.ref().update(updates).then(() => { alert(i18next.t('editor.delete_success')); state.allArticlesMeta = state.allArticlesMeta.filter(a => a.id != id); showMainView(); displayNewsList(state.allArticlesMeta); }).catch(e => alert(e.message));
            }
        };
...
// === Inicjalizacja TinyMCE ===
    ...
    function init() {
        ...
        auth.onAuthStateChanged(async u => {
            ...
            updateUserInfoFields(); calculatePermissions();
        });
        handleDeepLink();
    }
    init();
