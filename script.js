 ...
 * Wersja z optymalizacją i nasłuchiwaniem w czasie rzeczywistym.
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    let state = {
        ...
        allComments: [], // Będziemy przechowywać wszystkie załadowane komentarze
    ...
    // ZAKTUALIZOWANA FUNKCJA RENDEROWANIA KOMENTARZY
    function renderComments(comments) {
        const commentListContainer = elements.commentSection.list;
        commentListContainer.innerHTML = '';
        if (comments.length === 0) { commentListContainer.innerHTML = '<p>Brak komentarzy. Bądź pierwszy!</p>'; return; }
        ...
        // Delegacja zdarzeń - jeden nasłuchiwacz dla wszystkich przycisków
        if (commentListContainer.eventListener) { commentListContainer.removeEventListener('click', commentListContainer.eventListener); }
        const eventHandler = (event) => {
            const target = event.target;
            const commentEl = target.closest('.comment');
            if (!commentEl) return;
            const commentId = commentEl.dataset.commentId;
            if (target.classList.contains('delete-comment-btn')) { if (confirm("Czy na pewno chcesz usunąć ten komentarz?")) { database.ref(`comments/${state.currentArticle.id}/${commentId}`).remove(); } }
    ...
    function loadInitialArticles() { /* ... bez zmian ... */ }
    function loadMoreArticles() { /* ... bez zmian ... */ }
    
    // ZAKTUALIZOWANA LOGIKA KOMENTARZY (NASŁUCHIWANIE W CZASIE RZECZYWISTYM)
    function listenForComments(articleId) {
        if (state.commentsListener) state.commentsListener.off(); // Zawsze czyść stary nasłuchiwacz
        
        state.allComments = [];
        state.areAllCommentsLoaded = false;
        elements.loadMoreCommentsBtn.classList.remove('hidden');
        elements.loadMoreCommentsBtn.disabled = false;
        
        const commentsRef = database.ref(`comments/${articleId}`);
        state.commentsListener = commentsRef.on('value', (snapshot) => {
            const data = snapshot.val();
            state.allComments = data ? Object.entries(data).map(([key, val]) => ({ ...val, commentId: key })).sort((a,b) => b.timestamp - a.timestamp) : [];
            
            // Pokaż początkową liczbę komentarzy
            const initialComments = state.allComments.slice(0, COMMENTS_PER_PAGE);
            renderComments(initialComments);
            
            // Pokaż przycisk "Więcej", jeśli jest co ładować
    ...
    function loadMoreComments() {
        const currentlyShown = document.querySelectorAll('#comments-list .comment').length;
        const nextComments = state.allComments.slice(0, currentlyShown + COMMENTS_PER_PAGE);
        renderComments(nextComments);

        if (nextComments.length >= state.allComments.length) {
            elements.loadMoreCommentsBtn.classList.add('hidden');
        }
    }

    function setupLikes(articleId) { /* ... bez zmian ... */ }
    ...
    function initializeAuth() { /* ... bez zmian ... */ }
    function handleAdminLogin() { /* ... bez zmian ... */ }

    // =================================================================
    // === 7. FUNKCJE POMOCNICZE ========================================
    // =================================================================
    
    function getOrCreateLocalUserId() { /* ... bez zmian ... */ }
    function setupShareButton(article) { /* ... bez zmian ... */ }
    function handleDeepLink() { /* ... bez zmian ... */ }
    function parseCommentFormatting(text) { /* ... bez zmian ... */ }
    
    // =================================================================
    // === 8. OBSŁUGA ZDARZEŃ ===========================================
    // =================================================================
    
    function bindEventListeners() { /* ... bez zmian ... */ }
    ...
    function bindEventListeners() { elements.backButton.addEventListener('click', () => { if (state.commentsListener) state.commentsListener.off(); showMainView(); }); elements.slider.container.addEventListener('click', handleArticleClick); elements.newsList.addEventListener('click', handleArticleClick); elements.adminButton.addEventListener('click', () => { if (state.isUserAdmin) elements.adminMenu.container.classList.toggle('hidden'); else showView(elements.views.login); }); elements.adminMenu.addButton.addEventListener('click', () => { showEditor(null); elements.adminMenu.container.classList.add('hidden'); }); elements.adminMenu.logoutButton.addEventListener('click', () => { auth.signOut().then(() => { elements.adminMenu.container.classList.add('hidden'); alert("Wylogowano."); }); }); elements.loginForm.submitButton.addEventListener('click', handleAdminLogin); elements.loginForm.cancelButton.addEventListener('click', () => showView(null)); elements.commentSection.form.addEventListener('submit', (e) => { e.preventDefault(); const name = elements.commentSection.nameInput.value.trim(); const message = elements.commentSection.messageInput.value.trim(); if (name && message) { addComment(name, message); elements.commentSection.form.reset(); } }); window.addEventListener('hashchange', handleDeepLink); elements.loadMoreArticlesBtn.addEventListener('click', loadMoreArticles); elements.loadMoreCommentsBtn.addEventListener('click', loadMoreComments); elements.clearCacheBtn.addEventListener('click', () => { let clearedCount = 0; for (let i = localStorage.length - 1; i >= 0; i--) { const key = localStorage.key(i); if (key && key.startsWith('article_')) { localStorage.removeItem(key); clearedCount++; } } alert(`Wyczyszczono ${clearedCount} zapisanych artykułów.`); }); }
