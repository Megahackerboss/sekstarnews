document.addEventListener('DOMContentLoaded', () => {
    ...
    // === 4. BEZPIECZNY POŚREDNIK (Netlify Function) ===
    async function apiRequest(action, id, body) {
        try {
            ...
            console.error(`Błąd zapytania do API (${action}):`, error);
            return false;
        }
    }
    ...
    // === 6. GŁÓWNA LOGIKA APLIKACJI (Odczyt danych) ===
    ...
    // === 7. LOGIKA INTERAKCJI (Zapis danych przez API) ===
    function updateLikeButton(likes, articleId) {
        ...
        if (alreadyLiked) { likeButton.classList.add('liked'); likeButton.querySelector('.heart-icon').textContent = '♥'; } else { likeButton.classList.remove('liked'); likeButton.querySelector('.heart-icon').textContent = '♡'; }
        ...
        likeButton.onclick = async () => {
            ...
            if (success) {
                // Nie musimy nic robić - Firebase sam zaktualizuje liczbę
    ...
    commentForm.onsubmit = async (e) => {
        ...
        if (nameInput.value && messageInput.value) {
            const success = await apiRequest('comments', currentArticle.id, { author: nameInput.value, message: messageInput.value });
            if (success) {
                commentForm.reset();
            } else {
    ...
    // === 8. LOGIKA EDYTORA (Zapis bezpośrednio do Firebase, bo jesteś adminem) ===
