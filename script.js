document.addEventListener('DOMContentLoaded', () => {
    ...
    // === 4. LOGIKA LOGOWANIA I STANU ADMINA ===
    ...
    adminButton.addEventListener('click', () => {
        if (loggedIn) {
            showEditor(null); // Zalogowany: przycisk '+' otwiera edytor
        } else {
            loginView.classList.remove('hidden'); // Niezalogowany: przycisk '?' otwiera logowanie
        }
    });
    ...
    function loadArticlesFromFirebase() { /* ... bez zmian ... */ }
    function displayArticle(articleId) { /* ... bez zmian ... */ }
    ...
    commentForm.onsubmit = (e) => { /* ... bez zmian ... */ };
    function showEditor(article = null) { /* ... bez zmian ... */ }
    editorForm.addEventListener('submit', (e) => { /* ... bez zmian ... */ });
    editorCancelButton.addEventListener('click', () => editorView.classList.add('hidden'));
    editorDeleteButton.addEventListener('click', () => { /* ... bez zmian ... */ });
    ...
    function displayArticle(articleId) { currentArticle = allArticles.find(a => a.id == articleId); if (!currentArticle) return; if (commentsListener) commentsListener.off(); articleDate.textContent = currentArticle.date; articleAuthor.textContent = `Autor: ${currentArticle.author}`; articleContent.innerHTML = currentArticle.content; mainView.classList.add('hidden'); articleView.classList.remove('hidden'); backButton.classList.remove('hidden'); navTitle.style.marginLeft = '0px'; clearInterval(slideInterval); getLikes(articleId); listenForComments(articleId); }
    ...
    // === INICJALIZACJA APLIKACJI (NAPRAWIONA) ===
    function init() {
