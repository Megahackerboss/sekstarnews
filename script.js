document.addEventListener('DOMContentLoaded', () => {
    ...
    const shareButton = document.getElementById('share-button'); // <-- NAPRAWIONE: Dodano brakującą zmienną
    ...
    auth.onAuthStateChanged(user => { if (user && !user.isAnonymous) { loggedIn = true; adminButton.textContent = "≡"; } else if (!user) { auth.signInAnonymously().catch(error => console.error("Błąd logowania anonimowego:", error)); } });
    ...
    function addComment(author, message) {
        ...
        newCommentRef.set({
            author: author,
            message: message,
            userId: localUserId, // <-- Upewniamy się, że to jest tutaj
    ...
    // NAPRAWIONA FUNKCJA setupShareButton
